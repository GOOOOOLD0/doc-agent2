...
5. 读取 `wiki/concepts/landing_rights/cases/brazil/00_brazil_case_index.md`；
6. 读取 `wiki/concepts/landing_rights/cases/brazil/01_brazil_landing_overview.md`；
7. 读取 `wiki/concepts/landing_rights/cases/brazil/09_brazil_reusable_experience.md`；
8. 根据用户问题，选择性读取巴西案例中的具体模块文件；
9. 对目标国家进行开放式资料检索；
10. 识别目标国家自己的监管体系；
11. 将目标国家流程映射到通用分析框架；
12. 与巴西流程进行对比；
13. 输出结论、风险点、待确认问题和下一步建议。
